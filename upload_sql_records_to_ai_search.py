...
search_client = SearchClient(
    ...
    index_name=os.getenv("AI_SEARCH_NAME"),
    credential=AzureKeyCredential(AzureAiSearchConfig.API_KEY),
)
...
def clean_record_summary(record_summary):
    # TODO: Maybe we need more cleaning here, depends what OpenAI Embedding Models say
...
def embed_text(cleaned_record_summary):
    response = openai_client.embeddings.create(
        ...
        dimensions=OpenAiConfig.EMBEDDING_MODEL_DIMENSION  # Standard dimension for text-embedding-3-small model
    )
...
def ingest_all_records(sql_query):
    ...
    while True:
        ...
        for record in records:
            ...
            # TODO: Uncomment to enable full processing
